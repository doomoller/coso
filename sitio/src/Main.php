...
class Main
{
	...
	private function loadConfig($c)
	{
		 ...
		 */
		if(!isset($c['routers']))throw new Exception("Error: faltan los routers.");
		$this->routers = array();
		foreach ($c['routers'] as $k => $v)
		{
			$this->routers[$k] = new Router($k, $v);
		}
		if(!isset($this->routers['home']))throw new Exception("Error: home router.");
		if(!isset($this->routers['no-found']))throw new Exception("Error: no-found router.");
	...
	public function run()
	{
		...
		{
		    throw new \Exception("Error: No hay Response.");
		}
		
	}
	private function getController($controller, $controller_path)
	{
		if(!file_exists($controller_path))throw new Exception("Error: controladora: $controller_path");
		
		require_once $controller_path;
		
		if(!class_exists($controller))throw new Exception("Error: no existe ".$controller." Controller.");
